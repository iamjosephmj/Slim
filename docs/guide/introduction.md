# 1. Introduction — your first kernel

You're going to write your first ARM64 NEON kernel right now. By the
end of this chapter:

- You'll have a working kernel that brightens any float buffer by a
  constant factor.
- You'll have run it and seen the **~7×** speedup over the Kotlin
  scalar version on real hardware.
- You'll be able to read the disassembled output and understand what
  each line of your DSL actually compiled to.

No prior assembly experience required. The NEON instructions you'll
meet are introduced as you encounter them.
...
This is closure capture in action. Section 3 of the Cookbook covers
the mechanics in depth; for now, the takeaway is: any Kotlin
expression that produces a number can flow into the kernel as a
constant.
...
## 1.8 What just happened? (under the hood)
...
You've now written, run, and measured your first NEON kernel. Where
to next:

- **[Cookbook](../COOKBOOK.md)** — long-form integration guide. How
  `slim {}` composes with `suspend`, `Flow`, `Channel`, structured
  concurrency. How to pass complex Kotlin context (lookup tables,
  computed parameters) into kernels and get results back. The "SIMD as
  ordinary Kotlin" thesis and the patterns that follow from it. If
  you only read one more thing, read this.
- **Chapter 2: Registers and arrangements** *(coming)* — every
  register name in scope, what `B16`/`H8`/`S4`/`D2` actually mean
  bit-by-bit, when to use which arrangement, signed vs unsigned
  byte semantics.
- **Chapter 3: Branches, labels, and control flow** *(coming)* —
  `cbnz`/`cbz`/`tbnz`/`tbz`, the 16 condition codes, how forward and
  backward branches work, structured loops, early exits.
- **Chapter 4: Memory access patterns** *(coming)* — `ld1`/`st1`
  variants, addressing modes (offset / pre-indexed / post-indexed),
  alignment requirements, when to use `ldp`/`stp` for paired loads,
  when to use `ld1r` for replicating loads.

Until those chapters land, the [Cookbook](../COOKBOOK.md) recipes are
worked examples that exercise most of the same material in context.

---

*This is the end of Chapter 1. Feedback welcome via a
